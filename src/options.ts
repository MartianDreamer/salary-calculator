import { BASE_SALARY, GIAM_TRU_GIA_CANH, GIAM_TRU_GIA_CANH_NGUOI_PHU_THUOC, MINIMUM_SALARY, TAX_RANGES, DEFAULT_LOCALE } from './const';
...
const optBaseSalaryElement = document.getElementById('opt-base-salary') as HTMLInputElement;
const optPersonalDeductionElement = document.getElementById('opt-personal-deduction') as HTMLInputElement;
const optDependentDeductionElement = document.getElementById('opt-dependent-deduction') as HTMLInputElement;
const optRegion1Element = document.getElementById('opt-region-1') as HTMLInputElement;
const optRegion2Element = document.getElementById('opt-region-2') as HTMLInputElement;
const optRegion3Element = document.getElementById('opt-region-3') as HTMLInputElement;
const optRegion4Element = document.getElementById('opt-region-4') as HTMLInputElement;
const taxSettingsTableBody = document.getElementById('taxSettingsTableBody') as HTMLTableSectionElement;
const deleteAllButton = document.getElementById('deleteAllButton') as HTMLButtonElement;
const addNewButton = document.getElementById('addNewButton') as HTMLButtonElement;
const resetOptionButton = document.getElementById('resetOptionButton') as HTMLButtonElement;
const insertedEndElement = document.getElementById('insertedEnd') as HTMLInputElement;
const insertedValueElement = document.getElementById('insertedValue') as HTMLInputElement;
const insertConfirmElement = document.getElementById('insertConfirm') as HTMLButtonElement;
...
const loadOptionsToUI = () => {
    ...
    optPersonalDeductionElement.value = options.personalDeduction.toLocaleString(DEFAULT_LOCALE);
    optDependentDeductionElement.value = options.dependentDeduction.toLocaleString(DEFAULT_LOCALE);
...
export const handleOptions = () => {
    ...
    optBaseSalaryElement.addEventListener('input', () => {
        options.baseSalary = parseInt(optBaseSalaryElement.value.replace(/\./g, ''));
        saveToLocalStorage();
    });

    optPersonalDeductionElement.addEventListener('input', () => {
        options.personalDeduction = parseInt(optPersonalDeductionElement.value.replace(/\./g, ''));
        saveToLocalStorage();
    });

    optDependentDeductionElement.addEventListener('input', () => {
        options.dependentDeduction = parseInt(optDependentDeductionElement.value.replace(/\./g, ''));
        saveToLocalStorage();
    });

    optRegion1Element.addEventListener('input', () => {
        options.minimumSalary[0] = parseInt(optRegion1Element.value.replace(/\./g, ''));
        saveToLocalStorage();
    });

    optRegion2Element.addEventListener('input', () => {
        options.minimumSalary[1] = parseInt(optRegion2Element.value.replace(/\./g, ''));
        saveToLocalStorage();
    });

    optRegion3Element.addEventListener('input', () => {
        options.minimumSalary[2] = parseInt(optRegion3Element.value.replace(/\./g, ''));
        saveToLocalStorage();
    });

    optRegion4Element.addEventListener('input', () => {
        options.minimumSalary[3] = parseInt(optRegion4Element.value.replace(/\./g, ''));
        saveToLocalStorage();
    });
    ...
    insertedEndElement.addEventListener('input', () => {
        if (isNaN(parseInt(insertedEndElement.value.replace(/\./g, '')))) {
            ...
            insertedEndElement.value = parseInt(insertedEndElement.value.replace(/\./g, '')).toLocaleString(DEFAULT_LOCALE);
        }
    });

    insertConfirmElement.addEventListener('click', (e) => {
        e.preventDefault();
        const lastRange = options.taxRanges.length > 0 ? options.taxRanges[options.taxRanges.length - 1] : null;
